...
public class PlayerManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (attackboss ==false && attack==false)
        {
            ...
            {
                transform.GetChild(i).transform.GetComponent<Rigidbody>().isKinematic=true;  //kinematik kapanır
            }
        }
        else
        {
        ...
        if (attack)
        {
            ...
            var enemyDirection = new Vector3(enemy.position .x, transform.position.y,enemy.position.z)- transform.position;  //düşman konumunu alır

            for (int i = 1; i < transform.childCount; i++)
            {
                transform.GetChild(i).rotation = Quaternion.Lerp(transform.GetChild(i).rotation ,Quaternion.LookRotation(enemyDirection, Vector3.up),Time.deltaTime *fighttime); //konuam doğru çöpadamları çevirir
            }
            ...
            if (enemy.GetChild(1).childCount > 1)
            {
                for (int i = 1; i < transform.childCount; i++)
                {
                    var Distance = enemy.GetChild(1).GetChild(0).position - transform.GetChild(i).position; //aradaki mesafeyi hesaplar
                    ...
                    if (Distance.magnitude < DistancetoEnemy)
                    {
                        ...
                        enemy.GetChild(1).GetChild(0).position.z),Time.deltaTime *playercollidespeed);  //eğer mesafe girilen değerden küçükse admalar saldırmaya başlar
                    }
            ...
            {   
                ...
                FormatStickman();  //tekrardan pozisyon alırlar mavi çöpadamlar
                ...
                for (int i = 0; i < transform.childCount; i++)
                {
                    ...
                    anim.SetBool("EnemyZoneanim",true);     //enemyzone kırmızı animasyonu çalışır
            ...
            {
                enemy.transform.GetChild(1).GetComponent<EnemyManager>().StopAttack();  // eğer çöpadam kalmazsa animasyonları kapatır
        ...
        else
        {
            ...
            MoveThePlayer();  //hareket etme fonksiyonu
            gameState=true;
        ...
        {
          road.Translate(road.forward * Time.deltaTime * roadSpeed);  // yolun düz bir şekilde hareket etmesini böylece çöp adamların koşmasını sağlar
        }  
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
        if(bosszone)
        {
            ...
            for (int i = 1; i < transform.childCount; i++)  //eğer boss alanı içerisindeysek bossa doğru mavi çöpadamarın dönmesini sağlar
            {
        ...
        if (attackboss)
            {
                ...
                {for (int i = 1; i < transform.childCount; i++)
                {
                    ...
                    if (Distance.magnitude < DistancetoBoss)
                    {
                        ...
                        boss.GetChild(0).position.z),Time.deltaTime *(playercollidespeed/5));  //boss a saldırmayı sağlar
                    }
                }
                }
                else
    ...
    void MoveThePlayer()
    {
        if(Input.GetMouseButtonDown(0)&&gameState)
        {
            ...
            var ray = camera1.ScreenPointToRay(Input.mousePosition);  //mouse hareketleriyle karakterlerin ekranda sağa ve sola hareket etmesini sağlar.
        ...
        {
            var plane = new Plane(Vector3.up, 0f);
            var ray = camera1.ScreenPointToRay(Input.mousePosition);  //dokunmatik olarak telefonalrda çalışmasını sağlar
            ...
            if(plane.Raycast(ray, out var distance))
            {
                ...
                var control = playerStartPos + move;
                    ...
                    else if(30 <= numberofstickman && numberofstickman < 40)   // eçöp adam sayısı arttıkça ekrana sığmasını sağlar
    ...
    public void FormatStickman()
    {
        ...
        for (int i = 1; i < player.childCount; i++)
        {
            ...
            var x =DistanceFactor * Mathf.Sqrt(i) * Mathf.Cos(i*Radius);  //çöp adamların posizyonlarını girilen değere göre belirli bir geometrik şekilde düzenlemesini sağlar
    ...
    private void MakeStickman(int number)  //çöp adam yaratma fonksiyonudur.
    {
    ...
    {   
        ...
        {
            other.transform.parent.GetChild(0).GetComponent<BoxCollider>().enabled=false;  // eğer kapıalrdan geçilirse adam eklenmesi ve çarpılmasını tetikler
        ...
        if (other.CompareTag("enemy"))
        {
            ...
            other.transform.GetChild(1).GetComponent<EnemyManager>().AttackThem(transform);  //eğer düşmana çarparsak bunu tetikleyip saldırı komutunu çalıştırır.
    ...
    {
        numbersofenemystickman =enemy.transform.GetChild(1).childCount -1;  //çöpadam sayısını günceller
        numberofstickman = transform.childCount-1;
        ...
        while (numbersofenemystickman >0 && numberofstickman > 0)
        {
            ...
            enemy.transform.GetChild(1).GetComponent<EnemyManager>().Countertxt.text = numbersofenemystickman.ToString(); //düşman sayısını günceller
            Countertxt.text = numberofstickman.ToString();
    ...
    {
        GameObject PopUpClone = Instantiate(PopUpTxt, transform.position, Quaternion.identity);  // ekranda beliren sayı kodunu çalıştırır kod kapıdan geçince çalışır.

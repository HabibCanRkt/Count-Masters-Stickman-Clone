...
public class EnemyManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Instantiate(stickman,transform.position,new Quaternion(0f,180f,0f,1f),transform);  //20-35 adet arasında çöpadam yaratır
        }

        Countertxt.text =(transform.childCount ).ToString();        //yaratılan çöpadam sayısını ekrana yazar
        FormatStickman();
    ...
    void Update()
    {
       ...
       if (attack && transform.childCount > 1)
       {
         ...
         var EnemyPos =new Vector3(enemy.position.x,transform.position.y,enemy.position.z);  // eğer çöpadamlar saldırı moduna geçerse çalıır ve karşı çöp adamlara doğru önce koşup sonra saldırma komutunu çalıştırır.
    ...
    {
        for (int i = 0; i < transform.childCount; i++)   //çöp adamların tekrardan pozisyon almasını sağlar 
        {
    ...
    public void AttackThem ( Transform enemyForce)  //saldırı komutunu aktif eder
    {
    ...
    {
       PlayerManager.PlayerManagerInstance.gameState =attack = false;  // saldırı komutunu durdurur

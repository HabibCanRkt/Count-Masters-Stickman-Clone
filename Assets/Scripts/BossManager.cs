...
public class BossManager : MonoBehaviour
{
    ...
    void Update()
    {
        fillamount=(currenthealth / bossHealth);        //Boss un can barını kontrol eder.
        ...
        StartCoroutine(BossRun());          //Boss u koşturur
        ...
        if (currenthealth < 1)
        {
            ...
            transform.parent.GetChild(2).GetChild(1).transform.gameObject.SetActive(false);  //Boss ölünce çalışır
    ...
    {
        currenthealth = currenthealth -1;       //Boss a damage vermeye yarar
       
    }

   IEnumerator BossRun()
   {
        if(PlayerManager.PlayerManagerInstance.bosszone && PlayerManager.PlayerManagerInstance.numberofstickman > 0)  //Bossu koşturur
        {
